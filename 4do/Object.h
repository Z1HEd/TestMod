...
namespace fdo
{
	...
	public:
		...
		void tetrahedralize(std::vector<Point>& pos,
			...
			std::vector<uint32_t>& indexBuffer)
		{
			...
			for (int tetraIndex = 0; tetraIndex < tetrahedra.size();++tetraIndex)
				for (int vertIndex = 0; vertIndex < 4; ++vertIndex) {
					pos.push_back(vertices[tetrahedra[tetraIndex].vIndices[vertIndex]]);
					norm.push_back(normals[tetrahedra[tetraIndex].vnIndices[vertIndex]]);
					uvw.push_back(texCoords[tetrahedra[tetraIndex].vtIndices[vertIndex]]);
					col.push_back(colors[tetrahedra[tetraIndex].coIndices[vertIndex]]);
					indexBuffer.push_back(tetraIndex * 4 + vertIndex);
				}
		}
	};
}
